...
// Procura no container por elementos cuja chave é k e retorna
// o número de elementos encontrados. Porque HashTable não permite
// chaves duplicadas, isso significa que a função realmente retorna 1 se
// existe um elemento com essa chave no contêiner e zero, caso contrário.
template <typename K, typename V>
...
// Retorna o número de baldes na HashTable.
// Um balde é um slot na tabela de hash interna dos arrays
// aos quais os elementos são atribuídos com base no valor de hash de sua chave.
// A quantidade de caçambas influencia diretamente no fator de carga do
// tabela de hash do contêiner (e, portanto, a probabilidade de colisão).
template <typename K, typename V>
...
// Retorna o número de elementos no balde n.
// Um balde é um slot na tabela de hash interna dos arrays
// aos quais os elementos são atribuídos com base no valor de hash de sua chave.
// O número de elementos em um balde influencia o tempo que leva
// para acessar um elemento específico no balde.
template <typename K, typename V>
...
// Retorna o número do bucket onde o elemento com a chave k está localizado ou deveria estar contido.
template <typename K, typename V>
...
// Todos os elementos na HashTable são descartados:
// seus destruidores são chamados, e eles são removidos do container,
// deixando com tamanho 0.
template <typename K, typename V>
...
// Define o número de baldes no contêiner (_table_size) para
// o mais apropriado para conter pelo menos n elementos.
// Se n for maior que o _table_size atual multiplicado por
// o _load_factor, então a função rehash() é chamada com o argumento apropriado.
// Se n for menor que isso, a função pode não ter efeito.
template <typename K, typename V>
...
// define o fator de carga para um novo valor.
// o argumento lf deve ser um número positivo, lf > 0, e deve ser tal que lf <= _max_load_factor.
// se nenhuma dessas restrições for satisfeita, a função pode não ter efeito.
// no entanto, se as condições forem satisfeitas, o fator de carga é definido, ou seja, _load_factor = lf,
// e a função reserve(_n) é chamada
template <typename K, typename V>
